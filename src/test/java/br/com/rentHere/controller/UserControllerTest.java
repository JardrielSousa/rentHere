...
public class UserControllerTest {
	...
	private static final String URL_DELETE = "http://localhost:8080/v1/rent/user/1";
	@Autowired
	...
	public void test04UpdateUserWithError() throws JsonProcessingException, Exception {
		mock.perform(put(URL)
				.contentType(APPLICATION_JSON)
				.content(objectMapper.writeValueAsString(new UserUpdateDto(null,"test user", "test@gmail.com", "9855555"))))
		.andExpect(status().is4xxClientError());
	}
	
	@Test
	public void test05DeleteUserWithSucess() throws JsonProcessingException, Exception {
		mock.perform(delete(URL_DELETE)
				.contentType(APPLICATION_JSON))
		.andExpect(status().is4xxClientError());
	}
	
	private UserDto newUser() {
		return new UserDto("test user", "test@gmail.com", "9855555");
	}
	
	private UserUpdateDto updateUser() {		
		return new UserUpdateDto(1L,"test user", "test@gmail.com", "9855555");
	}
}
